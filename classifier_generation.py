...
2) Construction and analyse predictif using RandomForest classifier
...
def get_preprocess_dataset(data):
    ...
    tag_text_list = ["p", "div", "label", "tr", "th", "b", "span", "strong", "title", "td", "li", "h1", "h2", "h3", "h4", "h5", "h6", "dd",
                     "dt", "mark", "em"]
...
def get_test_kw_inner_text_length_y(data):
    ...
    list = data[data['y'].apply(lambda x: True if x in "CEML__PAGE__DESCRIPTION__LIST__ITEMS" else False)].inner_text_length
    noisy = data[data['y'].apply(lambda x: True if x in "NOISY" else False)].inner_text_length
    ...
    M = pd.DataFrame(M, columns=['CEML__TITLE', 'CEML__PRICE', 'CEML__DESCRIPTION', 'CEML__PAGE__DESCRIPTION__LIST__ITEMS', 'NOISY'])
    H, pval = mstats.kruskalwallis(M['CEML__TITLE'].tolist(), M['CEML__PRICE'].tolist(), M['CEML__DESCRIPTION'].tolist(),
                                   M['CEML__PAGE__DESCRIPTION__LIST__ITEMS'].tolist(), M['NOISY'].tolist())
...
def get_test_kw_child_text_length_y(data):
    ...
    title = data[data['y'].apply(lambda x: True if x == "CEML__TITLE" else False)].child_text_length
    price = data[data['y'].apply(lambda x: True if x == "CEML__PRICE" else False)].child_text_length
    desc = data[data['y'].apply(lambda x: True if x == "CEML__DESCRIPTION" else False)].child_text_length
    list = data[data['y'].apply(lambda x: True if x == "CEML__PAGE__DESCRIPTION__LIST__ITEMS" else False)].child_text_length
    noisy = data[data['y'].apply(lambda x: True if x == "NOISY" else False)].child_text_length
    ...
    H, pval = mstats.kruskalwallis(M['CEML__TITLE'].tolist(), M['CEML__PRICE'].tolist(), M['CEML__DESCRIPTION'].tolist(),
                                   M['CEML__PAGE__DESCRIPTION__LIST__ITEMS'].tolist(), M['NOISY'].tolist())
    print("Test Kruskal-Wallis for child_text_length grouped by y")
    print("H-statistic:", H)
    print("P-Value:", pval)
    if pval < 0.05:
        print("Reject NULL hypothesis - Significant differences exist between groups.")
...
def get_feature_selection_test_chi2(X, Y, data):
    ...
    # Features selection via test Chi2
    test = sfs.SelectKBest(score_func=sfs.chi2, k=5)
    fit = test.fit(X, Y)
