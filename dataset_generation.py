...
def get_raw_features(tag_list, tag_text_list):
    ...
    df_raw_features = pd.DataFrame(columns=['tag_name','element_id','class_name','children_names','children_ids','children_classes','image_alt','rect_size','num_child_elements','dom_subtree_depth','inner_text','child_text','ancestors_names','ancestors_ids','ancestors_classes','siblings_names','siblings_ids','siblings_classes','nearest_header','normalized_top','normalized_bottom','is_middle','is_leftmost','is_rightmost','num_siblings','distance_to_root','url','title','site','meta_description','doc_dom_depth'])
...
def get_derived_features(tag_list, tag_text_list):
    ...
    df_derived_features = pd.DataFrame(columns=['tag_name','has_children','inner_text_length','is_like_price','child_text_length','is_sib_p','is_sib_a','is_sib_input','is_desc_a','is_desc_nav','is_desc_ad','is_desc_comment','is_desc_main','is_desc_footer','is_desc_wrapper','is_desc_aside','is_desc_p','is_desc_div','is_desc_h','is_desc_ul','is_desc_table','is_desc_ol','is_desc_menu','contains_rights_reserved','contains_like','contains_share','is_link','is_thumbnail','has_depth_greater_2','all_desc_text_length','element_id','class_name','y'])
...
def get_derived_features_filtered(soup):
    ...
    df_derived_features = df_derived_features[~((df_derived_features.tag_name == 'p') & ((df_derived_features.is_desc_a == 1) | (df_derived_features.is_desc_nav == 1) | (df_derived_features.is_desc_ad == 1) | (df_derived_features.is_link == 1)))]
    df_derived_features = df_derived_features[~((df_derived_features.tag_name == 'div') & ((df_derived_features.is_desc_a == 1) | (df_derived_features.is_desc_nav == 1) | (df_derived_features.is_desc_ad == 1) | (df_derived_features.is_link == 1)))]
    df_derived_features = df_derived_features[~((df_derived_features.is_desc_ad == 1) & (df_derived_features.tag_name == 'li'))]
    df_derived_features = df_derived_features[~((df_derived_features.is_desc_nav == 1) & ((df_derived_features.tag_name == 'th') | (df_derived_features.tag_name == 'td')))]
    df_derived_features = df_derived_features[~((df_derived_features.tag_name.apply(lambda x: True if x in tag_text_list else False)) & (df_derived_features.inner_text_length == 0))]
    df_derived_features = df_derived_features[~((df_derived_features.tag_name.apply(lambda x: True if x in tag_header_list else False)) & ((df_derived_features.is_desc_a == 1) | (df_derived_features.is_desc_nav == 1) | (df_derived_features.is_desc_ad == 1) | (df_derived_features.is_link == 1)))]

    # delete rows in dataframe with the sectionning tags(tags with a depth greater 2 and contains a text)
    df_derived_features = df_derived_features[~((df_derived_features.tag_name.apply(lambda x: True if x in ['div','table','ol','ul','menu'] else False)) & (df_derived_features.has_depth_greater_2 == True) & (df_derived_features.all_desc_text_length > 0))]
